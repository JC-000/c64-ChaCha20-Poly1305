...
- `LIB_CHACHA20_POLY1305_RESIDENT_BYTES` — resident code+data upper bound, profile- and variant-aware. Rebased in v0.7.0 onto the library's own segment sum rather than whole-PRG size, each value rounded up to the next 256-byte boundary: Profile A full = 15872 (measured 15568), Profile A aead-only = 15360 (15243), Profile B full = 17152 (16841), Profile B aead-only = 16640 (16516), Profile B app-owned = 16896 (16544). **All five of these declared values are themselves too low** — they omit the inter-object alignment fill a consumer's link pays, and are short of a sound upper bound by 512 B. Tracked as [#113](https://github.com/JC-000/c64-ChaCha20-Poly1305/issues/113); do not size a region from them until that lands. **These literals are hand-maintained and nothing checks them** — three were pushed past their declared values by the §14.1 domain guards with no build diagnostic. The previous figures here (15544 / 16838 / 16513) were accurate at `v0.9.0`, which was byte-identical to `v0.7.0`, and went stale at `20e01aa` when the `aead_tag` fix added 11 B to every configuration. Re-measure by hand after adding code to any library TU; see the note in `src/lib/lib_manifest.s`.
