...
- `LIB_CHACHA20_POLY1305_RESIDENT_BYTES` — resident code+data upper bound, profile- and variant-aware: Profile A full **16384**, Profile A aead-only **16128**, Profile B full **17664**, Profile B aead-only **17408**, Profile B app-owned **17408**. **These are a BOUND, not a measurement** (issue #113): `od65` reports each object's segment sizes and cannot report the padding `ld65` inserts *between* sections, which this library forces to exist because `LIB_CHACHA20_POLY1305_CODE` must carry `align = $100` for its constant-time invariant. A consumer pays `sum + fill`, and fill is `(-offset) mod 256` per aligned section — so the declared value is `sum + 255 x (page-aligned sections)`, rounded up. Until v0.11.0 these were the raw sum rounded up, which left all five **512 B short** of what a consumer could pay, in the dangerous direction. Now pinned by `make verify-resident-bytes`; re-measure with `python3 tools/measure_resident_bytes.py <objdir>`.
- `LIB_CHACHA20_POLY1305_AEAD_ONLY_RESIDENT_BYTES` — tighter upper bound for consumers pinning the aead-only archive variant (Profile B 16640, measured 16516; Profile A 15360, measured 15243).
