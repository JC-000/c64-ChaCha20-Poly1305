...
1. Download the release tarball from the GitHub releases page:
   ...
   ```
   wget https://github.com/JC-000/c64-ChaCha20-Poly1305/releases/download/v0.8.0/c64-ChaCha20-Poly1305-v0.8.0.tar.gz
   ```
...
2. Unpack under `third_party/` in your consumer repo:
   ...
   ```
   mkdir -p third_party
   tar xzf c64-ChaCha20-Poly1305-v0.8.0.tar.gz -C third_party/
   # Result: third_party/c64-ChaCha20-Poly1305-v0.8.0/src/{lib,include,c64.cfg}
   ```
...
3. Point your consumer's ca65 invocation at the vendored `src/lib`
   ...
   ```
   CA65FLAGS = -t c64 -g \
       -I third_party/c64-ChaCha20-Poly1305-v0.8.0/src/lib \
       -I third_party/c64-ChaCha20-Poly1305-v0.8.0/src/include
   ```
...
git commit -m "vendor c64-chacha20poly1305 v0.8.0 as submodule"
```
...
**v0.8.0 (2026-08-13) requires a consumer cfg change.** The library
...
lines and why each one matters. v0.8.0 also stops exporting the §8.x
